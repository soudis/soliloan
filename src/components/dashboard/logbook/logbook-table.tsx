...
export function LogbookTable({ changes }: LogbookTableProps) {
  ...
  const getChangeDescription = (change: Change) => {
    const context = change.context as AuditContext;
    ...
    createColumn<Change>(
      {
        ...
        accessorFn: (row: Change) => {
          const context = row.context as AuditContext;
          return context.user?.name || t("unknownUser");
        },
        cell: ({ row }: { row: Row<Change> }) => {
          const context = row.original.context as AuditContext;
          return context.user?.name || t("unknownUser");
        },
      },
      t
    ...
    createColumn<Change>(
      {
        ...
        header: "when",
        cell: ({ row }: { row: Row<Change> }) => {
          return format(new Date(row.original.committedAt), "PPPp", {
            locale: de,
          });
        },
      },
