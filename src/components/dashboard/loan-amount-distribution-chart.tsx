...
export function LoanAmountDistributionChart({
  loans,
}: LoanAmountDistributionChartProps) {
  ...
  const filteredData = ranges
    .map((range) => {
      ...
      return {
        ...
        totalAmount: loansInRange.reduce(
    ...
    datasets: [
      {
        ...
        backgroundColor: filteredData.map(
          (item) => RANGE_COLORS[item.range as keyof typeof RANGE_COLORS]
        ),
