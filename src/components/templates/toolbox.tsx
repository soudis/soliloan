...
import { Image as ImageIcon, Layout, MousePointer2, Repeat, Type } from 'lucide-react';
...
import { Loop } from './user-components/loop';
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div
          ref={(ref) => {
            if (ref) connectors.create(ref, <Element is={Loop} loopKey="loans" canvas />);
          }}
          className="flex flex-col items-center justify-center p-3 border rounded-lg bg-zinc-50 hover:bg-zinc-100 cursor-move transition-colors group"
        >
          <Repeat className="w-5 h-5 mb-2 text-zinc-500 group-hover:text-zinc-900" />
          <span className="text-xs font-medium">{t('toolbox.loop')}</span>
        </div>
      </div>
    </div>
  );
