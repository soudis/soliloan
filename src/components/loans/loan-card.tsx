  ...
  return (
    <Card className={className}>
            ...
            <Badge
              variant={
                loan.status === LoanStatus.ACTIVE ? 'default' :
                  loan.status === LoanStatus.TERMINATED ? 'destructive' :
                    loan.status === LoanStatus.NOTDEPOSITED ? 'secondary' :
                      'outline'
              }
              className="mt-1"
            >
      ...
      <CardContent>
        ...
        />
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as 'transactions' | 'files' | 'notes' | 'bookings')} className="mt-6">
