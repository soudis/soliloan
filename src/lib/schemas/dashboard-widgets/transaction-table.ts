...
import {
  ...
  DEFAULT_TRANSACTION_TABLE_VISIBLE_COLUMNS,
...
export const transactionTableWidgetConfigSchema = z.object({
  layoutVersion: z.literal(1).default(1),
  columns: z
    .array(tableViewColumnSchema)
    .min(1)
    .max(200)
    .default(DEFAULT_TRANSACTION_TABLE_VISIBLE_COLUMNS.map((id) => ({ id, visible: true }))),
