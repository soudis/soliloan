...
export async function addFile(loanId: string, data: FileFormData, base64Data: string, mimeType: string) {
  try {
    ...
    if (mimeType.startsWith('image/') || mimeType === 'application/pdf') {
      try {
        ...
        if (mimeType === 'application/pdf') {
          ...
          await execAsync(`convert -density 300 "${tempInputPath}[0]" -background white -alpha remove -alpha off -resize 384x384^ -gravity center -extent 384x384 -quality 90 "${tempOutputPath}.png"`)
          ...
          await execAsync(`convert "${tempInputPath}" -resize 384x384^ -gravity center -extent 384x384 "${tempOutputPath}"`)
        }
    ...
    return { error: error instanceof Error ? error.message : 'Failed to create file' }
  }
} 